...
async def pause(_, message: Message):
    if (
        ...
        await message.reply_text(f"**{BN} :-** 🙄 Hiçbir şey oynamıyor.!")
    else:
...
async def stop(_, message: Message):
    if message.chat.id not in callsmusic.pytgcalls.active_calls:
        await message.reply_text(f"**{BN} :-** 🙄 Hiçbir şey devam etmiyor.!")
    else:
        try:
...
async def skip(_, message: Message):
    if message.chat.id not in callsmusic.pytgcalls.active_calls:
        ...
        await message.reply_text(f"**{BN} :-** 😬 Geçerli şarkı Geçildi!")
